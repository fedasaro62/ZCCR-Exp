...
class Data:
    def __init__(self,
        ...
        with open(os.path.join(self.dataset_root, '{}_classified_tags.csv'.format(dataset,threshold)), "r") as f:
            ...
            for row in tqdm(reader):
                ...
                _, _, label, _, img_tags, txt_tags = row
                if label not in data.keys():
                    data[label] = [[img_id, img_tags, txt_id, txt_tags]]
                else:
                    data[label].append([img_id, img_tags, txt_id, txt_tags])
                counter += 1
                # if counter >= 10000:
                #     break

        clusters                = np.array([id for id, _ in 
    ...
    def get_query_one_seed(self, class_:str):
        ...
        return pair_list[:self.n_points_per_cluster], pair_list[self.n_points_per_cluster]
    ...
    def get_query_multiple_seed(self, classes:List):
        ...
        return np.array(query_out), np.array(rel_out)
    ...
    def get_retrieval_item(self, i):
        ...
        '''
        img_id, img_tags, txt_id ,txt_tags  = self.retrieval_data[i]
        return img_id, img_tags, txt_id, txt_tags